	...
	int *addr;
	{
		...
		if (token == Num) {
			...
			*++text = token_val;
			...
			data = (char *)(((int)data + sizeof(int)) & ((-1)|sizeof(int)));
			expr_type = PTR;
		}
...
int main(int argc, char **argv)
{
	...
	if (fopen_s(&FileHandle,"d:\\RobotFile\\Intepreter.txt","r") != 0)
	{
	...
	if (!(src = (char*)malloc(poolsize))) {
		printf("could not malloc(%d) for src code\n", poolsize);
		system("pause");
		return -1;
	}
	memset(src, 0, poolsize);

	//if( fgets(src,poolsize,FileHandle) == NULL)
	//{
	//	printf("problem fget file, errno(%d)\n", errno);
	//	system("pause");
	//	return -1;
	//}

	if ((i = fread_s(src,poolsize,1,poolsize,FileHandle)) <= 0)
	{
		printf("problem fread_s file, errno(%d)\n", errno);
		system("pause");
		return -1;
	}
